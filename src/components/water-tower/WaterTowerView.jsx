...
import {
  calcTower, getEffTier, projectFuture, capLinePx, CONTAINER_H,
          ...
          <div className={
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {t.msg && <div className="text-[11px] text-stone-300 mt-0.5">{t.msg}</div>}
              </div>
            </div>
          </div>
        </div>
...
function SignModal({ player, totalCapHit, faSigned, faLimit, mleUsed, mleAmount, hardCapped, onConfirm, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[9998]" onClick={onCancel}>
      <div className="bg-[#0e1218] border border-stone-700 rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="text-center">
          <h3 className="text-lg font-black text-white">{player.name}</h3>
          <div className="text-sm text-stone-400 font-mono mt-1">{player.position} · R{player.rating} · Age {player.age}</div>
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-mono font-black text-stone-500 uppercase">契約年数</label>
        ...
        {mleAmount > 0 && !mleUsed && (
          <label className="flex items-center gap-2 text-xs text-cyan-400 font-mono">
            <input type="checkbox" checked={useMLE} onChange={e => setUseMLE(e.target.checked)} className="accent-cyan-500" />
            MLE使用 (${(Math.min(salary, mleAmount) / 1e6).toFixed(1)}M)
          </label>
        )}
        <div className="bg-stone-950 rounded-xl p-3 text-xs font-mono space-y-1">
        ...
        <div className="flex gap-2">
          <button onClick={onCancel} className="flex-1 bg-stone-900 border border-stone-800 text-stone-400 font-mono font-black py-2.5 rounded-xl text-sm">キャンセル</button>
          <button onClick={() => onConfirm(years, useMLE)} disabled={!canSign}
            className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-600 disabled:from-stone-800 disabled:to-stone-800 disabled:text-stone-600 text-stone-950 font-mono font-black py-2.5 rounded-xl text-sm">
            契約
          </button>
        </div>
      </div>
...
function WaterWave({ bottom }) {
  return (
    <div className="absolute left-0 right-0 pointer-events-none overflow-hidden" style={{ bottom: bottom - 6, height: 12 }}>
      <svg className="tw-wave" style={{ width: '200%', height: '100%' }} viewBox="0 0 200 12" preserveAspectRatio="none">
        <path d="M0,6 Q10,0 20,6 T40,6 T60,6 T80,6 T100,6 T120,6 T140,6 T160,6 T180,6 T200,6 V12 H0 Z" fill="rgba(6,182,212,0.25)" />
      </svg>
    </div>
  );
}
...
function CapMarker({ label, value, dashColor }) {
  return (
    <div className="absolute left-0 right-0 border-t border-dashed opacity-25" style={{ bottom: capLinePx(value), borderColor: dashColor }}>
      <span className="absolute left-2 -top-4 text-[9px] font-mono text-stone-500">{label} ${(value / 1e6).toFixed(0)}M</span>
    </div>
  );
}
...
function SeasonSummary({ season, record, summaries, onContinue }) {
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[9998]">
      <div className="bg-[#0e1218] border border-stone-700 rounded-2xl p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-2xl space-y-4">
        <div className="text-center space-y-2">
          <span className="text-xs font-mono font-black text-cyan-400 uppercase tracking-widest">SEASON {season} REPORT</span>
          {record && (
            <div className="space-y-1">
              <div className="text-2xl font-mono font-black text-white">{record.wins}勝 {record.losses}敗</div>
              <div className={'text-lg font-mono font-black ' + (record.gmBonus > 0 ? 'text-emerald-400' : 'text-stone-500')}>
                {record.result} {record.gmBonus > 0 && `+${record.gmBonus} GM SCORE`}
              </div>
            </div>
          )}
        </div>
        <div className="bg-stone-950 rounded-xl p-4 space-y-1 max-h-60 overflow-y-auto">
          <h3 className="text-xs font-mono font-black text-stone-400 uppercase mb-2">選手変化</h3>
          {summaries.map((s, i) => (
            <div key={i} className="flex justify-between items-center text-sm py-1">
              ...
              <span className="font-mono">
                ...
                {s.change !== 'RETIRE' && <span className="text-red-500 ml-1 text-xs">({s.change})</span>}
              </span>
            </div>
          ))}
        </div>
        <button onClick={onContinue} className="w-full bg-gradient-to-r from-cyan-500 to-blue-600 text-stone-950 font-mono font-black py-3 rounded-xl text-sm">
          ドラフトへ →
        </button>
      </div>
    </div>
...
export default function WaterTowerView({ onBack, gmName, playClickSound, isBgmOn, toggleBGM }) {
  ...
  function handleBuyout(player) {
    ...
    if (Math.random() * 100 < chance) {
      ...
      const nd = [...deadCapDetails, { name: player.name + ' (B/O)', amount: dead, yearsLeft: player.contractYears, type: 'Buyout' }];
      setDeadCapDetails(nd);
      setRoster(r => r.filter(p => p.id !== player.id));
      setSelectedBlockId(null);
      ...
      playError();
      addToast('warning', '❌', `バイアウト拒否`, `${player.name} (確率${chance}%)`, 3000);
    }
  }
  ...
  function handleStretch(player) {
    ...
    const nd = [...deadCapDetails, { name: player.name + ' (ST)', amount: annual, yearsLeft: stretchYears, type: 'Stretch' }];
    setDeadCapDetails(nd);
    setRoster(r => r.filter(p => p.id !== player.id));
    setSelectedBlockId(null);
  ...
  /* ── Header ── */
  const Header = () => (
    <header className="px-6 py-3 flex justify-between items-center border-b border-stone-800/50 shrink-0">
      <div className="flex items-center gap-3">
        <button onClick={() => { playClickSound(); onBack(); }} className="text-stone-500 hover:text-stone-300 font-mono text-sm px-2 py-1">🏠</button>
        <h1 className="text-xl font-black font-mono text-cyan-400 tracking-wider">💧 WATER TOWER</h1>
        <span className="text-sm font-mono text-stone-500">SEASON {season}</span>
      </div>
      <div className="flex items-center gap-4">
        <button onClick={() => { playClickSound(); toggleBGM(); }}
          className={'px-2 py-1 rounded text-sm ' + (isBgmOn ? 'text-emerald-400' : 'text-stone-500')}>{isBgmOn ? '🔊' : '🔇'}</button>
        <span className="text-[10px] font-mono text-stone-600">GM SCORE</span>
  ...
  function Block({ block, isSelected, isSubmerged, onClick }) {
    return (
      <div onClick={onClick}
        className="absolute left-6 right-6 rounded-lg cursor-pointer transition-all duration-200 tw-block-in group"
        style={{
          bottom: block.yBottom, height: block.height,
          borderLeft: `4px solid ${block.tier.color}`,
        ...
        <div className="flex items-center justify-between px-4 h-full overflow-hidden">
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-white font-bold text-sm truncate">{block.player.name}</span>
            <span className="text-[10px] bg-stone-800/80 text-stone-400 px-1 rounded font-mono shrink-0">{block.player.position}</span>
            {block.player.contractType === 'rookie' && <span className="text-[9px] bg-emerald-950 text-emerald-400 px-1 rounded font-mono shrink-0">RK</span>}
            {block.player.birdRights === 'Full' && <span className="text-[9px] bg-blue-950 text-blue-300 px-1 rounded shrink-0">🐦</span>}
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <span className="font-mono font-black text-base" style={{ color: block.tier.color }}>{block.player.rating}</span>
            <div className="text-right">
              <div className="text-stone-300 font-mono text-[11px]">${(block.player.salary / 1e6).toFixed(1)}M</div>
              <div className="text-stone-600 font-mono text-[10px]">{block.player.contractYears}yr · {block.tier.label}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  function ActionBar({ player }) {
    ...
    return (
      <div className="flex gap-1.5 items-center bg-stone-950/90 border border-stone-800 rounded-lg px-3 py-2 shrink-0">
        <span className="text-white font-bold text-sm mr-2">{player.name}</span>
        <button onClick={() => handleWaive(player)}
          className="text-[10px] bg-amber-950/80 border border-amber-700 text-amber-300 px-2 py-1 rounded font-mono hover:bg-amber-900">💀 ウェイブ</button>
        {player.contractYears > 1 && (
          <button onClick={() => handleBuyout(player)}
            className="text-[10px] bg-purple-950/80 border border-purple-700 text-purple-300 px-2 py-1 rounded font-mono hover:bg-purple-900">🤝 B/O</button>
        )}
        {player.contractYears > 1 && (
          <button onClick={() => handleStretch(player)}
            className="text-[10px] bg-emerald-950/80 border border-emerald-700 text-emerald-300 px-2 py-1 rounded font-mono hover:bg-emerald-900">⏳ ST</button>
        )}
        <button onClick={() => setSelectedBlockId(null)} className="text-stone-500 hover:text-white text-xs ml-1">✕</button>
      </div>
    );
  }
          ...
          <div className="bg-[#0c0f16] border border-stone-800/50 rounded-xl p-4" style={{ width: 360, height: 320 }}>
            <div className="relative w-full h-full overflow-hidden">
              ...
              <div className="absolute left-0 right-0 border-t-2 border-dashed border-amber-500/40 tw-pulse" style={{ bottom: tower.ratingLinePx * 0.5 }}>
                <span className="absolute right-1 -top-4 text-[9px] font-mono text-amber-400">Rating {tower.ratingLine}</span>
              </div>
              {tower.blocks.map((b, i) => (
                <div key={b.id} className="absolute left-4 right-4 rounded-sm tw-block-in"
                  style={{ bottom: b.yBottom * 0.5, height: Math.max(6, b.height * 0.5), borderLeft: `3px solid ${b.tier.color}`, backgroundColor: `${b.tier.color}18`, animationDelay: `${i * 30}ms` }} />
              ))}
            </div>
          </div>
          <div className="bg-stone-950 border border-stone-800 rounded-xl px-6 py-3 font-mono text-sm text-stone-400 flex gap-6">
            <span>Total: <span className="text-white font-black text-lg">{tower.totalRating}</span></span>
            <span>Cap: <span className={tower.isUnderCap ? 'text-cyan-400 font-black text-lg' : 'text-red-400 font-black text-lg'}>${(tower.totalCapHit / 1e6).toFixed(1)}M</span></span>
            <span>Players: <span className="text-white font-black">{roster.length}</span></span>
          </div>
          <div className="flex gap-3">
            <button onClick={doReroll} className="bg-stone-900 border border-stone-800 text-stone-300 font-mono font-black px-6 py-2.5 rounded-xl text-sm hover:bg-stone-800 transition-all">🔄 REROLL</button>
            <button onClick={() => { playClickSound(); setPhase('manage'); }} className="bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-stone-950 font-mono font-black px-8 py-2.5 rounded-xl text-sm transition-all">START 💧</button>
            <button onClick={() => { playClickSound(); onBack(); }} className="bg-stone-900 border border-stone-800 text-stone-500 font-mono font-black px-4 py-2.5 rounded-xl text-sm hover:text-stone-300">← 戻る</button>
          </div>
        </div>
      </div>
    );
  ...
  if (phase === 'manage') {
    ...
    return (
      <div className="h-screen bg-[#080b10] text-white font-sans antialiased flex flex-col selection:bg-cyan-500 selection:text-black overflow-hidden">
        ...
        {Header()}
        <main className="flex-1 flex overflow-hidden px-4 py-3 gap-3 min-h-0">
          {/* ══ Tower Area ══ */}
          <div className="flex-1 flex flex-col min-w-0 gap-2">
            ...
            <div className="h-10 flex items-center shrink-0">
              {selectedPlayer ? (
                ...
                <span className="text-stone-600 font-mono text-xs">← ブロックをクリックして放出操作</span>
              )}
            </div>
          </div>
          ...
          <div className="w-80 flex flex-col gap-2 shrink-0 min-h-0">
            ...
            <div className="bg-[#0e1218] border border-stone-800/50 rounded-xl flex-1 flex flex-col min-h-0 overflow-hidden">
              <div className="flex justify-between items-center px-3 pt-3 pb-2 shrink-0">
                ...
                <button onClick={refreshFA} className="text-[10px] font-mono text-stone-600 hover:text-stone-300 bg-stone-900 px-1.5 py-0.5 rounded">🔄</button>
              </div>
              <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-1">
                {freeAgents.map(p => {
                  const tier = getEffTier(p.rating, p.salary);
                  return (
                    <div key={p.id} className="bg-stone-950/50 border border-stone-800/40 rounded-lg p-2 hover:border-stone-700 transition-colors">
                      <div className="flex items-center justify-between">
                        <span className="text-white text-xs font-bold truncate">{p.name}</span>
                        <span className="font-mono font-black text-xs shrink-0 ml-2" style={{ color: tier.color }}>{p.rating}</span>
                      </div>
                      <div className="flex items-center justify-between mt-0.5">
                        <span className="text-stone-500 font-mono text-[10px]">{p.position} · ${(p.salary / 1e6).toFixed(1)}M · {p.contractYears}yr · {tier.label}</span>
                        <button onClick={() => handleSignRequest(p)} disabled={faSignedThisSeason >= faLimit}
                          className="text-[10px] bg-cyan-950/60 border border-cyan-800 text-cyan-300 px-1.5 py-0.5 rounded font-mono hover:bg-cyan-900 disabled:opacity-30 disabled:cursor-not-allowed shrink-0 ml-1">
                          ＋
                        </button>
                      </div>
                    </div>
  ...
  if (phase === 'seasonEnd') {
    return (
      <div className="min-h-screen bg-[#080b10] text-white font-sans antialiased flex flex-col items-center justify-center">
        ...
        {!showSummary && (
          <button onClick={handleToDraft} className="bg-gradient-to-r from-cyan-500 to-blue-600 text-stone-950 font-mono font-black py-3 px-8 rounded-xl">ドラフトへ →</button>
        )}
      </div>
    );
  }
  ...
  if (phase === 'draft') {
    return (
      <div className="min-h-screen bg-[#080b10] text-white font-sans antialiased flex flex-col items-center justify-center px-6">
        {CSS}
        <ToastContainer toasts={toasts} />
        <div className="w-full max-w-2xl space-y-4 bg-[#0e1218] border border-stone-800 rounded-2xl p-8">
          <div className="text-center space-y-2">
            <span className="text-xs font-mono font-black text-cyan-400 uppercase tracking-widest">💧 DRAFT</span>
            <h2 className="text-3xl font-black">新人ドラフト</h2>
            <p className="text-stone-400 font-mono text-sm">残りピック: <span className="text-cyan-400 font-black">{picksLeft}</span></p>
          </div>
          {picksLeft > 0 ? (
              ...
              {draftProspects.map((p, i) => {
                ...
                return (
                  <div key={p.id} className="bg-stone-950 border border-stone-800 rounded-xl p-3 flex items-center justify-between hover:border-cyan-800 transition-colors">
                    <div>
                      <span className="text-stone-600 font-mono text-xs mr-2">#{i + 1}</span>
                      <span className="text-white font-bold">{p.name}</span>
                      <span className="text-stone-500 font-mono text-xs ml-2">{p.position} · R{p.rating}{p.pot > p.rating ? ` Pot${p.pot}` : ''} · Age {p.age}</span>
                      <span className="font-mono text-xs ml-2" style={{ color: tier.color }}>${(p.salary / 1e6).toFixed(1)}M · {tier.label}</span>
                    </div>
                    <button onClick={() => handleDraft(p)} className="bg-cyan-950 border border-cyan-700 text-cyan-400 hover:bg-cyan-900 font-mono font-black px-4 py-1.5 rounded-lg text-sm">DRAFT</button>
                  </div>
                );
              })}
            </div>
            ...
            <div className="text-center py-8 space-y-3">
              <div className="text-emerald-400 font-mono font-black text-xl">✓ ドラフト完了</div>
              <div className="text-stone-400">Rating: {totalRating} · Cap: ${(totalCapHit / 1e6).toFixed(1)}M</div>
              <button onClick={handleDraftComplete} className="w-full bg-gradient-to-r from-cyan-500 to-blue-600 text-stone-950 font-mono font-black py-3 rounded-xl text-sm">
                新シーズン開始 💧
              </button>
            </div>
          )}
          {picksLeft > 0 && (
            <button onClick={handleDraftComplete} className="w-full bg-stone-900 border border-stone-800 text-stone-500 font-mono font-black py-2.5 rounded-xl text-sm hover:text-stone-300">
              スキップ →
            </button>
          )}
        </div>
  ...
  if (phase === 'gameOver') {
    return (
      <div className="min-h-screen bg-[#080b10] text-white font-sans antialiased flex flex-col items-center justify-center px-6">
        {CSS}
        <div className="w-full max-w-lg space-y-5 bg-[#0e1218] border border-red-900 rounded-2xl p-8 text-center">
          <div className="space-y-1">
            <span className="text-xs font-mono font-black text-red-400 uppercase tracking-widest">TOWER COLLAPSED</span>
            <h2 className="text-3xl font-black">タワー崩壊</h2>
          </div>
          <div className="bg-stone-950 rounded-xl p-5 space-y-2">
          ...
          <div className="bg-stone-950 rounded-xl p-4">
            ...
            <div className="text-4xl font-black text-amber-400 font-mono">{gmScore}<span className="text-lg text-stone-600 ml-1">pts</span></div>
          </div>
          <div className="flex gap-3 justify-center">
            <button onClick={() => { doReroll(); }} className="bg-gradient-to-r from-cyan-500 to-blue-600 text-stone-950 font-mono font-black px-6 py-3 rounded-xl text-sm">TRY AGAIN 🔄</button>
            <button onClick={() => { playClickSound(); onBack(); }} className="bg-stone-900 border border-stone-800 text-stone-400 font-mono font-black px-4 py-3 rounded-xl text-sm hover:text-white">タイトルへ</button>
          </div>
        </div>
      </div>
    );
