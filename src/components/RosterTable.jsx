  ...
  return (
    <div className="flex-1 bg-[#141210] border border-stone-800 rounded-xl shadow-xl flex flex-col min-h-0">
      <div className="px-3 sm:px-4 py-2 sm:py-3 border-b border-stone-900 flex justify-between items-center shrink-0">
        ...
        <span className="text-[9px] sm:text-[10px] text-stone-500 font-mono">{players.length}人</span>
      </div>
        ...
        {players.map((player) => (
          <div key={player.id} className="bg-stone-950 border border-stone-900 rounded-lg p-2.5 flex items-center justify-between">
            <div className="flex-1 min-w-0">
              ...
              <div className="font-mono text-stone-500 text-[10px] mt-0.5">${player.salary.toLocaleString()}</div>
            </div>
            <button onClick={() => onActionClick(player)} className="bg-stone-900 border border-stone-800 text-stone-400 hover:text-white hover:border-stone-600 px-2.5 py-1.5 rounded-lg transition-colors font-mono text-[10px] shrink-0 ml-2">{actionLabel}</button>
          </div>
        ))}
      </div>
    </div>
